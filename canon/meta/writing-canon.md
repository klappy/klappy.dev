...
9. **Rendering-readiness test:** If this document is intended to appear on the site, does it have `public: true` AND `exposure: public`? The value `nav` is valid but means "navigable in catalog, not published on site." Compare against 2+ working published essays to verify. See `canon/constraints/frontmatter-validation-before-merge.md`.
