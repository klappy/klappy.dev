...
**Encoding types parse caching (0.18.0, encode).** `cachedEncodingTypes` holds the parsed `EncodingTypeDef[]` from walking 10+ encoding-type canon docs, extracting frontmatter, regex-matching `## Quality Criteria` tables, and parsing each row. Building this list per request would repeat markdown parsing for every encode call. Cached by `knowledgeBaseUrl` — invalidates cleanly when the *URL* changes. It does not notice a new commit at the same URL; see **Same-URL freshness** under the plumbing tax (amended 2026-09-21). *Status note, 2026-09-21: oddkit 0.23.0 later removed this encode cache entirely, citing this principle; this paragraph still describes it as kept. That disagreement between this document and the shipped code is open and is not resolved by this amendment.*
