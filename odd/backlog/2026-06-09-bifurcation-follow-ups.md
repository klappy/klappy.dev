...
Remaining: **AGENTS.md two-way split** (creed/axioms half → core; oddkit-MCP-integration half → oddkit) — still scope-map "undecided". Smell-pass findings 2026-06-09 (klappy-baked-in vs klappy-as-proof): klappy-as-proof docs stay core per criteria (vodka-architecture, scoped-truth, consistency, community-checking, specs-lock et al.). Actions: odd/getting-started/odd-agents-and-mcp moved to oddkit (it is setup instructions — user manual per criterion 4); governance-validation-via-agents genericized in core ([OWNER]/[REPO] placeholders). Flagged, pending maintainer: stakes-calibration split (mode mechanism + software modes core; writing-lifecycle modes overlay); frontmatter-validation-before-merge genericization pass (klappy.dev renderer war stories). Oddkit-boundary trio ruled core 2026-06-09: envelope-time-fields (the principle — request time vs content provenance — travels; oddkit field names are the worked example), canon-integration-audit, frontmatter-validation-before-merge.
