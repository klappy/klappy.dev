...
Six declared transitions cover the mode-discipline cycle and its two guarded escalations: opening planning after exploration, opening execution after planning, reverting from execution to exploration when the plan needs rethinking, declaring completion when execution produces a validated artifact, and — the guarded ones — handing anything off to the captain. The `dispatch-to-captain-handoff` transition exists because of the offload trip-wire in `klappy://canon/constraints/dispatcher-dispatches-never-executes`: a seat about to ask the captain to run/launch/commit/push/deploy anything runs its draft ask through the gate, and the gate demands proof that delegation to a subagent was attempted and that a human-only class (secret / voice / irreversible / approval) is named. Its detection vocabulary is deliberately rich in second-person tokens (you, your, captain) so BM25 routes offload phrasings here rather than to `execution-to-completion`, whose vocabulary shares operational verbs like deploy and push but carries no second-person signal. The `settled-direction-ratification` transition exists because of the let-the-gates-govern clause of the same canon constraint: a seat about to ask the captain to approve/ratify/bless/sign-off/green-light anything runs the draft ask through the gate, and the gate demands `open_fork_cited` — the ask must affirmatively name the genuinely-open fork the gates cannot resolve, because an ask that ratifies an already-recorded decision has no open fork to name and comes back NOT_READY. Its detection vocabulary is rich in approval stems (ratify, approve, bless, sign-off) and settled-record markers (already decided, per the ruling, settled call) that appear in neither the offload row (operational verbs) nor `execution-to-completion` (shipping verbs), so BM25 routes approval-shaped asks here. The word `proceed` appears exactly once in this row (in `shall I proceed`), deliberately: `proceed` is also a detection term of `exploration-to-planning`, and listing it more than once would let term frequency hijack a bare "proceed" input away from the planning transition — with a single occurrence, BM25 length normalization and row-order tiebreak keep bare "proceed" routing to `exploration-to-planning` as before. Each row carries its own detection vocabulary and prerequisite-id list, so consumers reading this file can answer "what fires this transition, and what must be true for it to pass?" in one lookup. Prerequisite ids resolve against `odd/gate/prerequisites.md` — that file defines each id's description, check vocabulary, and gap message. Detection terms are matched via BM25 over stemmed tokens; the specific-phrase-beats-single-word property of BM25 scoring means that `ready to build` outscores the bare word `ready` when both appear in the input, so the specific transition wins on score rather than relying on regex cascade order.
...
| `settled-direction-ratification` | execution | captain-escalation | open_fork_cited | ratify, ratification, rubber stamp, bless the decision, your blessing, do you approve, please approve, approve the direction, approve the plan, approve the decision, awaiting your approval, ready for your approval, your sign-off, sign off on, green-light this, re-approve, re-confirm, shall I proceed, already decided, already settled, per the ruling, settled call, direction is set |
